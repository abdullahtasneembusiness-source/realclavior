...
export async function inviteMember(
  ...
  const workspaceParsed = workspaceIdSchema.safeParse(workspaceId);
  ...
  if (!workspaceParsed.success) return { error: "Invalid workspace." };
  if (!parsed.success) {
  ...
  const workspaceName = await getWorkspaceName(supabase, workspaceId);
...
export async function changeRole(
  ...
  if (!parsedWs.success || !parsedId.success || !parsedRole.success) {
