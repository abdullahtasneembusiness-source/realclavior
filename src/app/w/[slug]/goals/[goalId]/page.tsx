...
export default async function GoalDetailPage({
  ...
  params: { slug: string; goalId: string };
}) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3">
        <Link
          href={`/w/${ctx.workspace.slug}/goals`}
      ...
      <GoalDetail workspaceId={ctx.workspace.slug} goalId={goal.id} goal={goal} />
          ...
          <div className="divide-y divide-border overflow-hidden rounded-xl border border-border bg-card">
            {playbooks.map((p) => {
              ...
              return (
                <Link
                  key={p.id}
                  href={`/w/${ctx.workspace.slug}/playbooks/${p.id}`}
