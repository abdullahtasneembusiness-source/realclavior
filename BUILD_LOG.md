...
- **Service-role client.** New `src/lib/supabase/admin.ts` (service-role key, no session) so the job sees every workspace, bypassing RLS. Guarded behind the secret check — never imported into user-privileged code.
