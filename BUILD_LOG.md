...
- **The cron gap, handled honestly (now closed — see "Launch auto-spawn cron" below).** At Session 9 there was no scheduler, so spawn fired at arm-time when due plus an opportunistic `checkDueLaunches` catch-up on page load. That left future-dated launches dependent on someone opening the app. The scheduled job now exists and is the real production trigger; arm-time + catch-up remain as belt-and-suspenders.
...
## Launch auto-spawn cron (reliability follow-up to Session 9)

Closed the one real reliability gap from Launch Mode: a launch scheduled for a future date only spawned its runs when a founder happened to open the launches page. Now a scheduled job fires it on its day, untouched.

- **Endpoint.** `GET /api/cron/launches` — finds every armed, non-template launch with `start_date <= today` across **all** workspaces and spawns each via the shared `spawnLaunch`, flipping it live. Auth is a Bearer `CRON_SECRET`; it fails closed if the secret is unset, and rejects any mismatch with 401 so the public can't trigger it.
- **Service-role client.** New `src/lib/supabase/admin.ts` (service-role key, no session) so the job sees every workspace, bypassing RLS. Guarded behind the secret check — never imported into user-privileged code. **It forces `cache: "no-store"` on its fetch** — without that, Next.js caches supabase-js's requests inside the Route Handler and the cron re-reads a just-flipped launch as still `armed`, re-spawning its runs every tick (a real double-assignment bug, caught by the E2E cron test via server-side logging; the authenticated in-app path was unaffected).
- **Idempotency is atomic.** `spawnLaunch` claims the launch by flipping `armed → live` FIRST via a conditional update that returns the row only to the caller that won the transition; only then does it spawn. Combined with the fresh reads above, a retried/overlapping tick can't double-spawn.
- **Shared spawn core.** Extracted `computeDueAt` + `spawnLaunch` out of the launches server action into `src/lib/launches.ts` so the action path (RLS client) and the cron path (service-role client) run the *identical* snapshot logic. `logActivity` widened to accept either client — on the cron's userless client it no-ops. Idempotent: `spawnLaunch` only acts on an `armed` launch and flips status guarded on `status='armed'`, so a retried/overlapping cron never double-spawns.
- **New migration.** `20260717190000_grant_service_role_access.sql` grants `service_role` the same public-schema table access `authenticated`/`anon` already had — needed for the service-role client to get past Postgres's table permission check on the local/CI stack (hosted Supabase pre-grants this, so it's a no-op there).
- **Schedule.** `vercel.json` → daily at 07:00 UTC. Day-granularity is enough since spawning keys off `start_date`; each spawned run still carries its own `due_at` time.
- **E2E** (`launch-cron.spec`): arm a *future*-dated launch (stays armed, no dashboard), advance its start date to today, confirm an unauthenticated/wrong-secret call is 401 and changes nothing, then the real Bearer call spawns the run and flips it live — and a second call doesn't double-spawn. Real browser + real HTTP to the endpoint.

**Live-DB follow-up.** `20260717190000_grant_service_role_access.sql` is a no-op on hosted Supabase (service_role is pre-granted) but should be applied for parity. **Vercel setup:** set `CRON_SECRET` (any long random string) in the project's environment variables — the cron won't run without it — and the `vercel.json` cron registers automatically on deploy.
